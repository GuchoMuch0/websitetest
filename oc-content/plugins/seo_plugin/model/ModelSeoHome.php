...
class ModelSeoHome extends DAO {
    ...
    public function getTable_SeoAttr() {
        return DB_TABLE_PREFIX.'t_home_seo' ;
    }

    public function getAttrHome() {
        $this->dao->select();
        $this->dao->from( $this->getTable_SeoAttr());
        $this->dao->where('seo_home_id', 1 );
    ...
    public function insertAttr( $home_id, $title, $desc, $keywords ) {
        $aSet = array(
            'seo_title'  => $title,
    ...
    public function updateAttr( $home_id, $title, $desc, $keywords ) {
        $aSet = array(
            'seo_title'  => $title,
            'seo_description'  => $desc,
            'seo_keywords' => $keywords,
        ...
        $aWhere = array( 'seo_home_id' => $home_id);
