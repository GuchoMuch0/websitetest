...
/*
  ...
  Version: 1.1.0
  Author: Richard Martin (keny) & Osclass modified by san-osclass.com
 ...
 */
    ...
    function watchlist() {
        ...
        if ($watched) {
            echo '<a class="icon_star_prem" href="' . osc_base_url(true) . '?page=custom&file=watchlist/watchlist.php"  title="' . __('View your watchlist', 'watchlist') . '"><i class="flaticon-heart-1"></i></a>';
        } else {
        echo '<a href="javascript://" title="' . __('Add to watchlist', 'watchlist') . '" class="watchlist" id="' . osc_item_id() . '">';
        echo '<i class="flaticon-heart"></i>';
        echo '</a>';
        }
    }

	 function watchlist_prem() {
       $conn = getConnection();
        $watched = $conn->osc_dbFetchResult("SELECT *
		FROM  %st_item_watchlist
		WHERE fk_i_item_id = '%d'
		AND  fk_i_user_id = '%d'", DB_TABLE_PREFIX, osc_premium_id(), osc_logged_user_id());

        if ($watched) {
            echo '<a class="icon_star" href="' . osc_base_url(true) . '?page=custom&file=watchlist/watchlist.php"  title="' . __('View your watchlist', 'watchlist') . '"><i class="flaticon-heart-1"></i></a>';
        } else {
        echo '<a href="javascript://" title="' . __('Add to watchlist', 'watchlist') . '" class="watchlist" id="' . osc_premium_id() . '">';
        echo '<i class="flaticon-heart"></i>';
        echo '</a>';
        }
    }
	 ...
	 function watchlist_header() {
        ...
        echo '<a class="log-button" href="' . osc_route_url('watchlist-user', array('iPage' => null)) . '" >' . __('Watchlist', 'watchlist') . '</a>';
    }
    ...
    function watchlist_user_menu_route() {
        ...
        echo '<li class="opt_watchlist '.$class.'" ><a href="' . osc_route_url('watchlist-user', array('iPage' => null)) . '" ><i class="ion ion-ios-heart"></i>' . __('Watchlist', 'watchlist') . '</a></li>';
    }
