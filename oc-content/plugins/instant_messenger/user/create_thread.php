...
<?php if(osc_is_web_user_logged_in() && $item['fk_i_user_id'] <> osc_logged_user_id() || !osc_is_web_user_logged_in()) { ?>
  ...
  <form id="im-create-thread-form" name="im-create-thread-form" class="im-body im-form-validate" action="<?php echo osc_route_url( 'im-create-thread', array('item-id' => $item_id) ); ?>" method="POST" enctype="multipart/form-data">
    ...
    <div class="im-row">
      <div class="im-col-24">
        <label class="im-label" for="im-title"><?php _e('Conversation title', 'instant_messenger'); ?></label>
    ...
    <?php if(osc_get_preference('att_enable','plugin-instant_messenger') == 1) { ?>
  ...
  <?php if(count($threads) > 0) { ?>
    <div class="im-threads-exist im-body">
      ...
      <?php foreach($threads as $t) { ?>
        ...
        <a class="im-row im-has-tooltip-left" href="<?php echo osc_route_url( 'im-messages', array('thread-id' => $t['i_thread_id']) ); ?>" title="<?php _e('Open conversation', 'instant_messenger'); ?>">
