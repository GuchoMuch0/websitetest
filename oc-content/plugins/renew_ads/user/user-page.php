    ...
    <?php if( osc_item_is_enabled () ) { ?>
    	...
    	<?php if( osc_item_is_active () ) { ?>
            ...
            <div class="item" >
                    ...
                    <br /> <strong><?php echo $willExpire.$daysToExpire ; ?></strong>
		...
		<form  action="<?php echo osc_route_url('renew-ads-user-menu'); ?>" method="POST" >
			...
			<button id="renew_ads_form" type="submit" ><?php printf(__('%s this ad', 'renew_ads'), $republish); ?></button>
...
<br />
    ...
    <?php for($i = 0 ; $i < $total_pages ; $i++) {
        ...
        if($i == $page) {
            ...
            printf('<a class="searchPaginationSelected" href="%s">%d</a>', osc_route_url('renew-ads-user-menu', array('iPage' => $i)), ($i + 1));

        } else {

            printf('<a class="searchPaginationNonSelected" href="%s">%d</a>', osc_route_url('renew-ads-user-menu', array('iPage' => $i)), ($i + 1));
