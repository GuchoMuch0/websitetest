<?php
 ...
 <?php if( is_writable( osc_uploads_path()) ) { ?>
 	<div class="admin-content">
 		<h2 class="admin-title"><i class="far fa-image"></i> <?php _e('Footer logo preview', 'san_auto'); ?></h2>
 			<?php
 			    $logo_prefence2 = osc_get_preference('logo_footer', 'san_auto');
 			?>
 		<?php if($logo_prefence2) { ?>
 			<img border="0" class="logo-auto" alt="<?php echo osc_esc_html( osc_page_title() ); ?>" src="<?php echo san_auto_logo_url().'?'.filemtime(osc_uploads_path() . osc_get_preference('logo_footer','san_auto'));?>" />
 			<form action="<?php echo osc_admin_render_theme_url('oc-content/themes/san_auto/admin/footer-logo.php');?>" method="post" enctype="multipart/form-data" class="nocsrf">
