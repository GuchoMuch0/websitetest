    ...
    class BreadcrumbAuto
    {
            ...
            switch($this->getLocation()) {
                ...
                case('search'):
                        ...
                        if( $b_location ) {
                            ...
                            if( $b_city ) {
                                $sRegion = Region::newInstance()->findByName($region);
                                $aCity = City::newInstance()->findByName($city, $sRegion['pk_i_id']);
                                if( count($aCity) == 0 ) {
                                    $params['sCity'] = $city;
                                    $l = array(
        ...
        public function render($separator = '&raquo;')
        {
            ...
            for($i = 0; $i < count($this->aLevel); $i++) {
				$current = false;
				if($i == count($this->aLevel)-1){ $current = ' class="current" ';}
                $text = '<li '.$current ;
            ...
            $result  = '<ul class="breadcrumbs">' . PHP_EOL;
            $result .= implode('', $node);
            $result .= '</ul>' . PHP_EOL;
